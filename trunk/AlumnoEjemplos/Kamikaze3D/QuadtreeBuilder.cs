...
namespace AlumnoEjemplos.Kamikaze3D
{
    ...
    class QuadtreeBuilder
    {
        ...
        public QuadtreeNode crearQuadtree(List<TgcSkeletalMesh> TgcMeshs, TgcBoundingBox sceneBounds)
        {
        ...
        private void doSectorQuadtreeX(QuadtreeNode parent, Vector3 center, Vector3 size,
            int step, List<TgcSkeletalMesh> meshes)
        {

            float x = center.X;

            //Crear listas para realizar corte
            List<TgcSkeletalMesh> possitiveList = new List<TgcSkeletalMesh>();
            List<TgcSkeletalMesh> negativeList = new List<TgcSkeletalMesh>();
        ...
        private void doSectorQuadtreeZ(QuadtreeNode parent, Vector3 center, Vector3 size, int step,
            List<TgcSkeletalMesh> meshes, int childIndex)
        {
            float z = center.Z;
        ...
        /// </summary>
        private void splitByPlane(Plane cutPlane, List<TgcSkeletalMesh> modelos,
            List<TgcSkeletalMesh> possitiveList, List<TgcSkeletalMesh> negativeList)
        {
            TgcCollisionUtils.PlaneBoxResult c;
            foreach (TgcSkeletalMesh modelo in modelos)
            {
        ...
        private void optimizeSectorQuadtree(QuadtreeNode[] children)
        {
            ...
            for (int i = 0; i < children.Length; i++)
            {
                ...
                if (childNodeChildren != null && hasEmptyChilds(childNode))
                {
                    ...
                    childNode.models = new TgcSkeletalMesh[0];
                }
                else
                {
